 ...
 * Features: Supabase, budgets, trend, recurring, installment, insights, CSV
 */
(function () {
  'use strict';
  ...
  var CHART_COLORS = ['#7A3018','#A0621A','#3E4832','#5C1A30','#C48A3A','#5E6A50','#A04828','#C4896A','#8A9478','#7A2445','#2E2010'];
  ...
  // cat chart / expense list / bulk select
  ...
  var elTrendChart = $('trend-chart'), elCsvExportBtn = $('csv-export-btn');
  ...
  function addExpense() {
    clearError();
    var rawAmt=elAmt.value.trim(), cat=elCat.value, payment=elPayment.value, notes=elNotes.value.trim();
  ...
  function resetForm(){elAmt.value='';elCat.value='';elPayment.value='';elNotes.value='';elIsRecurring.checked=false;elIsInstallment.checked=false;elInstallmentMonthsRow.hidden=true;if(elInstallmentMonths)elInstallmentMonths.value='';elAmt.focus();}
